...
//      SLTrace("\n");
    ...
    // Only check for attitude. Local_position_ned is not needed and difficult to request with Arducopter
    if ( current_messages.time_stamps.attitude )
    {
      SLTrace("INITIAL ATTITUDE = [ %.4f , %.4f , %.4f ] \n",
          current_messages.attitude.roll * RAD_TO_DEGf ,
          current_messages.attitude.pitch * RAD_TO_DEGf ,
          current_messages.attitude.yaw * RAD_TO_DEGf );
      SLTrace("\n");
  ...
  case COMMSTATE_WRITE_INIT:
    ...
    sp.coordinate_frame = MAV_FRAME_LOCAL_OFFSET_NED;
    sp.vx       = 0.0;
    sp.vy       = 0.0;
    sp.vz       = 0.0;
    sp.yaw_rate = 0.0;
  ...
  case COMMSTATE_READ_WRITE:
    ...
    // Read is one bye at a time so run it faster than write
    for (int k = 0; k < 100; k++)
    {
      read_messages(ctxt);
    }
...
SLStatus SendSteeringCommandsToAP(f32 Xcmd, f32 Ycmd, f32 ClimbRateFraction)
{
	...
	sp.target_component = 0;//Broadcast.  Was: autopilot_id;

	sp.coordinate_frame = MAV_FRAME_BODY_OFFSET_NED;
	sp.type_mask = MAVLINK_MSG_SET_POSITION_TARGET_LOCAL_NED_VELOCITY; // preset to ignore pos & accel
...
static void read_messages(GcContext *ctxt)
{
	int success = 1;              // receive success flag
	//bool received_all = false;  // received set of messages
	Time_Stamps this_timestamps;
	memset(&this_timestamps, 0, sizeof(Time_Stamps));


	static bool debug = false;  // set true to output notification for each message received
#ifdef DEBUG
	debug= true;
#endif

	static bool heartbeatReceived = false;
	static bool sysStatusReceived = false;
	static bool attitudeReceived = false;
	static bool globalPositionIntReceived = false;
	...
	static bool rcChannelsNotHandledRaw = false;
	static bool gpsRawIntNotHandled = false;
	static bool missionCurrentNotHandled = false;
	static bool controllerOutputNotHandled = false;
	static bool powerStatusNotHandled = false;
		...
		if (success)
		{
			...
			switch (message.msgid)
			{
			  //SLTrace("msgid: %i\n", (int)message.msgid);
				...
				{
					if(debug) { SLTrace("MAVLINK_MSG_ID_BATTERY_STATUS\n"); }
				...
				{
					if(debug) { SLTrace("MAVLINK_MSG_ID_RADIO_STATUS\n"); }
				...
				{
					if(debug) { SLTrace("MAVLINK_MSG_ID_LOCAL_POSITION_NED\n"); }
				...
				{
					if(debug || !globalPositionIntReceived)
					{
						SLTrace("MAVLINK_MSG_ID_GLOBAL_POSITION_INT\n");
						globalPositionIntReceived = true;
					}
				...
				{
					if(debug) { SLTrace("MAVLINK_MSG_ID_POSITION_TARGET_LOCAL_NED\n"); }
				...
				{
					if(debug) { SLTrace("MAVLINK_MSG_ID_POSITION_TARGET_GLOBAL_INT\n"); }
				...
				{
					if(debug) { SLTrace("MAVLINK_MSG_ID_HIGHRES_IMU\n"); }
				...
				case MAVLINK_MSG_ID_ATTITUDE:
				{
					...
					//SLTrace("Roll: %f\n", (float)current_messages.attitude.roll*RAD_TO_DEGf);
					//SLTrace("Pitch: %f\n", (float)current_messages.attitude.pitch*RAD_TO_DEGf);

					// Timestamp receipt of autopilot data for landing aid
					SLGetMHzTime(&APTelemetryTimeStamp);
					//SLTrace("%i\n", APTelemetryTimeStamp);
          ...
          if(!rcChannelsNotHandledRaw)
          {
            SLTrace("Warning, RC_CHANNELS_RAW Not handled.  Suppressing further warnings.\n");
            rcChannelsNotHandledRaw = true;
          }
          ...
          break;
				...
				case MAVLINK_MSG_ID_GPS_RAW_INT:
					if (!gpsRawIntNotHandled)
					{
						SLTrace("Warning, GPS_RAW_INT Not handled.  Suppressing further warnings.\n");
						gpsRawIntNotHandled = true;
					}
					break;

				case MAVLINK_MSG_ID_MISSION_CURRENT:
					if (!missionCurrentNotHandled)
					{
						SLTrace("Warning, MISSION_CURRENT Not handled.  Suppressing further warnings.\n");
						missionCurrentNotHandled = true;
					}
					break;

				case MAVLINK_MSG_ID_NAV_CONTROLLER_OUTPUT:
					if (!controllerOutputNotHandled)
					{
						SLTrace("Warning, NAV_CONTROLLER_OUTPUT Not handled.  Suppressing further warnings.\n");
						controllerOutputNotHandled = true;
					}
					break;

				case MAVLINK_MSG_ID_POWER_STATUS:
					if (!powerStatusNotHandled)
					{
						SLTrace("Warning, POWER_STATUS Not handled.  Suppressing further warnings.\n");
						powerStatusNotHandled = true;
					}
					break;
...
int read_message(mavlink_message_t &message, GcContext *ctxt)
{
	...
	//   READ FROM PORT
	// --------------------------------------------------------------------------
	s32 result = ctxt->autopilotPort->Read(&cp, 1, 1);  //using SL read

	// If the serial buffer is empty return
	if (result == 0)
		return 0;
	...
	if(result > 0)
	{
	  //SLTrace("read_message: Read something from serial port\n");
		// Parse message one byte at a time
		msgReceived = mavlink_parse_char(MAVLINK_COMM_3, cp, &message, &status);
